    ...
    def create(self, request, *args, **kwargs):
        ...
        if password1.isdigit():
            ...
            cart_serializer = CartSerializer(
                data={'customer': customer_instance.id, "products": []})
...
class CustomerProfile(generics.RetrieveUpdateDestroyAPIView):
    '''
    This API gives permission to customers to retrieve, update and destroy his profile.
    '''
...
class CartUpdateView(generics.RetrieveUpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        cart_products_idlist = list(
            cart_products_list.values_list('pk', flat=True))
        if len(cart_products_idlist) != 0:
            for productthrough in cart_products_list:
                if productthrough.product_id == int(requested_productid):
                    if max(int(requested_quantity), int(productthrough.quantity)) > productthrough.product.stock:
                        raise NotAcceptable(
                            detail="We don't have enough stock.")
                    added_product = ProductOrder.objects.get(
                        product_id=requested_productid)
                    ProductOrder.objects.filter(product_id=int(requested_productid)).update(
                ...
                productthrough_serializer = ProductOrderSerializer(
                    data=request.data['products'])
                productthrough_serializer.is_valid(raise_exception=True)
                added_product = ProductOrder.objects.create(
                    product=requested_product, quantity=int(requested_quantity))
                Response(added_product.quantity)
                updated_products_set = set(
                    cart_products_idlist).union({added_product.id})
            ...
            productthrough_serializer.is_valid(raise_exception=True)
            added_product = ProductOrder.objects.create(
                product=requested_product, quantity=request.data['products']['quantity'])
...
class CartRemoveView(generics.UpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        if len(cart_products_idlist) != 0:
            for productthrough in cart_products_list:
                if productthrough.product_id == int(requested_productid):
                    to_remove_product = cart_data.products.get(
                        product_id=int(requested_productid))
                    if to_remove_product.quantity > int(requested_quantity):
                        cart_data.products.filter(product_id=int(requested_productid)).update(
                            quantity=to_remove_product.quantity - int(requested_quantity))
                        request.data['products'] = cart_products_idlist
                    else:
                        request.data['products'] = list(
                            set(cart_products_idlist).difference({to_remove_product.id}))
                        to_remove_product.delete()
                    break
            else:
                raise NotAcceptable(
                    detail="You don't have this item in your cart.")
...
class PlaceOrderView(generics.ListCreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if request.data.get('receiver_mobile_number') is None:
            raise NotAcceptable(
                detail="Receiver's mobile number not provided.")
        ...
        for product in list(cart_products):
            if product.quantity > product.product.stock:
                raise NotAcceptable(
                    detail=f"This product {product.product.name} is currently out of stock.")
            else:
                ProductOrder.objects.filter(id=product.id).update(
                    order_price=product.product.price*product.quantity)
                Product.objects.filter(id=product.product.id).update(
                    stock=product.product.stock - product.quantity)
        request.data['customer'] = customer.id
        request.data['products'] = list(
            cart_products.values_list('pk', flat=True))
        none_list = []
...
class ItemsCancelOrderView(generics.RetrieveUpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        pk = kwargs.get('pk')
        orderdatalist = list(
            PlaceOrder.objects.filter(customer_id=customer.id))
        ...
        ordered_products_set = set(
            ordered_products.values_list('pk', flat=True))
        productid_to_be_cancelled = int(request.data.get('products'))
        if productid_to_be_cancelled not in ordered_products_set:
            raise NotAcceptable(
                detail="You have not ordered this product yet.")
        cancelled_order_data = {
            "customer": customer.id,
            "products": [productid_to_be_cancelled]
        }
        cancelled_order_serializer = CancelledOrderSerializer(
            data=cancelled_order_data)
        cancelled_order_serializer.is_valid(raise_exception=True)
        cancelled_order_serializer.save()
        request.data['customer'] = customer.id
        remaining_products_set = ordered_products_set.difference(
            {productid_to_be_cancelled})
        cancelled_product_order = ProductOrder.objects.get(
            id=productid_to_be_cancelled)
        Product.objects.filter(id=cancelled_product_order.product.id).update(
            stock=cancelled_product_order.product.stock+cancelled_product_order.quantity)
...
class CancellOrderView(generics.ListCreateAPIView):
    def create(self, request, *args, **kwargs):
        ...
        for ordered_product in list(order_details.products.all()):
            Product.objects.filter(id=ordered_product.product.id).update(
                stock=ordered_product.product.stock + ordered_product.quantity)
