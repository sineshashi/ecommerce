...
class ProducSerializer(serializers.ModelSerializer):
    class Meta:
        model = Product
        fields = ['id', 'seller', 'product_tag', 'name', 'category', 'price', 'stock',
                  'description', 'additional_information', 'image', 'status', 'date_created']
...
class OrderSerializer(serializers.ModelSerializer):
    class Meta:
        model = PlaceOrder
        fields = ['customer', 'order_status', 'products',  'total_price', 'receiver_name', 'receiver_name',
                  'receiver_mobile_number', 'place', 'district', 'state', 'country', 'ordered_at', 'updated_at']
...
class CreateSellerSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Seller
        fields = ['seller', 'seller_name', 'store_address', 'mobile_number',
                  'selling_product_category', 'created_at', 'updated_at']
...
class UpdateSellerSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        if user_data is not None:
            instance.seller.username = user_data.get(
                'username', instance.seller.username)
            instance.seller.first_name = user_data.get(
                'first_name', instance.seller.first_name)
            instance.seller.last_name = user_data.get(
                'last_name', instance.seller.last_name)
            instance.seller.email = user_data.get(
                'email', instance.seller.email)
            instance.seller.save()
        instance.mobile_number = validated_data.get(
            'mobile_number', instance.mobile_number)
        instance.seller_name = validated_data.get(
        ...
        if validated_category is None:
            ...
            if already_selling_category is None:
                instance.selling_product_category.set(
                    validated_category, instance)
            else:
                already_selling_category_list = list(
                    seller_data.selling_product_category.values_list('pk', flat=True))
                validated_category = list(set(validated_category).union(
                    set(already_selling_category_list)))
                instance.selling_product_category.set(validated_category)
        instance.store_address = validated_data.get(
...
class RetrieveSellerSerializer(serializers.ModelSerializer):
    class Meta:
        model = Seller
        fields = ['seller', 'seller_name', 'total_income', 'store_address',
                  'mobile_number', 'selling_product_category', 'created_at', 'updated_at']
